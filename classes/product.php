...
class product
{
	...
	public function insert_product($date, $files)
	{
		...
		$file_temp = $_FILES['image']['tmp_name']; // Lấy đường dẫn tạm thời của tệp tin trên server.

		$div = explode('.', $file_name); // Phân chia tên tệp tin thành mảng bằng cách tách chuỗi khi gặp dấu chấm (.).
		$file_ext = strtolower(end($div)); //Lấy phần mở rộng của tệp tin và chuyển nó thành chữ thường.
		$unique_image = substr(md5(time()), 0, 10) . '.' . $file_ext;
		$uploaded_image = "uploads/" . $unique_image; //Xây dựng đường dẫn đầy đủ đến thư mục "uploads" với tên tệp tin mới tạo.
	...
	public function insert_slider($date, $files)
	{
		...
		if ($sliderName == "" || $type == "") {
			...
			if (!empty($file_name)) {
				...
				$result = $this->db->insert($query);
				if ($result) {
					$alert = "<span class='success'>Thêm silder thành công</span>";
					return $alert;
				} else {
	...
	public function show_product()
	{
		...
		$result = $this->db->select($query);
		return $result;
		//lấy thông tin chi tiết về sản phẩm, bao gồm tên danh mục và tên thương hiệu, từ ba bảng khác nhau và sắp xếp kết quả theo productId giảm dần.
	...
	public function del_product($id)
	{
		...
		if ($result) {
			...
			$alert = "<span class='success'>Xoá sản phẩm không thành công</span>";
			return $alert;
		}
	}
	...
	public function getLastestHublot()
	{
	...
	public function insertWishlist($productid, $customer_id)
	{
		...
		if ($result_check_wlist) {
			$msg = "<span class='error'>Sản phẩm thêm vào danh sách yêu thích</span>";
			return $msg;
		} else {
